...
1. **哲学层**：我们究竟在设计什么？
2. **神经生物学映射**：大脑-小脑架构与数字神经系统
3. **结构层**：它是什么？——精密的层级调度体系
4. **行为层**：它如何工作？——Think-Act-Observe 自主循环
5. **进化层**：它如何成长？——从工具到思想熔炉的跨越
6. **交互与安全层**：确认型决策流程与信任分级
7. **代码智能矩阵**：四大核心 Agent 的详细规范

---

## 🧠 二、神经生物学映射：大脑-小脑架构

Cortex 的设计逻辑与人类神经系统的功能分工高度吻合。这个类比能帮我们更深刻地理解框架的运作逻辑，并解释之前的设计矛盾。

### 2.1 大脑皮层 (Meta-Agent & Core Intelligence)

这是思想产生、规划、决策和全局认知的地方。

| 组件 | 神经生物学对应 | 功能比喻 |
| :--- | :--- | :--- |
| **ConceptParser / IntentParser** | 前额叶皮层 | **意识与意图形成**。将模糊的想法转化为明确的指令和计划。 |
| **MissingInfoInferrer** | 前额叶 + 记忆系统 | **认知补全**。利用过去的经验来推断当前模糊情境中缺失的信息。 |
| **BlueprintGenerator** | 前额叶 + 运动前区 | **行动计划生成**。将意图分解为可执行的、有顺序的具体步骤序列。 |
| **AgentSelector** | 基底节 | **习惯与技能选择**。基于历史成功率和当前目标，自动选择最合适的执行回路。 |
| **Meta-Agent (整体)** | 整个大脑皮层 | **全局工作空间**，负责综合信息、做出决策、调度资源。 |

### 2.2 小脑 (Sub-Agent System & Autonomous Execution)

这是负责精细化执行、技能自动化和实时反馈调整的地方。

| 组件 | 神经生物学对应 | 功能比喻 |
| :--- | :--- | :--- |
| **CodeGenerator** 等 Sub-Agent | 小脑深部核团 | **精细化执行**。接收大脑指令，协调一系列精细的工具调用（读写文件、执行命令）来完成任务。 |
| **SkillExecutor** | 小脑皮层回路 | **自动化技能**。执行已经固化为YAML模板的技能序列，无需大脑皮层参与思考。 |
| **TaskDecomposer** | 小脑 | **动作分解与编排**。将高级任务（写一个模块）分解为低级动作（创建文件、写函数、写测试）。 |
| **Think-Act-Observe 循环** | 小脑-大脑反馈回路 | **实时纠错**。在执行为过程中不断检查结果，并自动调整下一步动作。 |
| **ErrorRecovery** | 小脑纠错机制 | **自动修复**。遇到标准错误时，不假思索地执行预设的修复程序。 |

### 2.3 边缘系统 (Memory & Evolution)

这是记忆、情感和进化动力的核心。

| 组件 | 神经生物学对应 | 功能比喻 |
| :--- | :--- | :--- |
| **Episodic Memory** (操作记忆) | 海马体 | **情景记忆**。记录“何时、何地、做了什么、结果如何”的事件。 |
| **Concept Memory** (概念记忆) | 大脑皮层联络区 | **语义记忆**。存储从情景记忆中抽象出来的知识、概念和事实。 |
| **Skill Memory** (技能记忆) | 小脑 + 纹状体 | **程序性记忆**。存储“如何做一件事”的运动模式或操作序列。 |
| **ProactiveEngine** | 杏仁核 + 下丘脑 | **情绪与动机**。基于当前状态和记忆，产生主动服务的动机和“直觉”。 |

### 2.4 自主神经系统 & 感觉器官 (Infrastructure & Tools)

这是维持系统生存、连接外界和提供基础功能的地方。

| 组件 | 神经生物学对应 | 功能比喻 |
| :--- | :--- | :--- |
| **EventBus** | 神经系统的电化学信号 | **神经通路**。连接所有脑区，让信息能在不同模块间以领域事件的形式高速传递。 |
| **ToolGateway** | 运动神经元通路 | **执行网关**。将大脑/小脑的行动指令传递给肌肉/工具，并进行安全校验。 |
| **FileTool / ShellTool** | 手 / 口 / 工具 | **效应器/肌肉**。执行具体操作的终端工具，与环境进行物理交互。 |
| **CompletionAgent** | 脊髓反射弧 | **膝跳反射**。对特定模式（代码前缀+后缀）的即时、最低延迟的自动化反应。 |
| **SystemAgent** | 自主神经系统 | **心跳与呼吸**。在后台无意识地监控和维持整个系统的健康状态。 |

### 2.5 信任与免疫系统 (Security)

这是保护机体免受伤害的防线。

| 组件 | 神经生物学对应 | 功能比喻 |
| :--- | :--- | :--- |
| **信任分级模型** | 免疫系统的“自我/非我”识别 | **渐进式授权**。熟悉的对象自动放行，陌生或危险的对象则触发检查。 |
| **CommandInterceptor** | 血脑屏障 / 免疫细胞 | **安全屏障**。主动拦截和杀死危险病原体（恶意命令），只允许已知安全的物质进入。 |
| **TaskToken** | 特定抗体 | **一次性授权令牌**。针对特定任务发放有时效性的权限，防止权限滥用。 |

**这个比喻模型清晰地解释了：**
*   **为什么 Agent 不能直接互相调用**：因为神经信号必须通过中枢（大脑）或指定的通路（小脑），不能随意串扰。
*   **为什么信任不能跨领域迁移**：因为对“手臂肌肉”的信任，不会自动转移到对“发声器官”的信任——它们是不同的神经通路和效应器。
*   **为什么技能系统需要 `strict_mode`**：因为小脑的自动化回路是快速且刻板的，如果需要灵活性，必须让大脑皮层介入。
