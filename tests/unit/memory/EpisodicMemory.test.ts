...
describe('EpisodicMemory', () => {
  ...
  describe('sanitizeSearchQuery', () => {
    ...
    it('should preserve Chinese characters', () => {
      const sanitized = episodicMemory['sanitizeSearchQuery']('测试中文');
  ...
  describe('意图感知与自适应权重', () => {
    ...
    describe('getAdaptiveWeights', () => {
      it('时间敏感查询应增强时间权�?, () => {
        ...
        expect(weights.t).toBeGreaterThan(0.2); // 时间权重应高于基础�?
        expect(weights.k + weights.t + weights.e + weights.v).toBeCloseTo(1, 5); // 归一�?
      });

      it('实体敏感查询应增强关键词和实体权�?, () => {
        const weights = episodicMemory['getAdaptiveWeights']('calculateTotal函数');
        expect(weights.k).toBeGreaterThan(0.3); // 关键词权重增�?
    ...
    describe('searchSemantic混合检�?, () => {
      beforeEach(async () => {
        // 初始化索�?
        await episodicMemory.initialize();
      });

      it('时间指代查询应返回最近记�?, async () => {
  ...
  describe('retrieve() - 分支覆盖', () => {
    ...
    it('应该支持sinceTimestamp过滤', async () => {
      const sinceTs = Date.now() - 3600000; // 1小时�?
  ...
  describe('searchSemantic() - TF-IDF检�?, () => {
    beforeEach(() => {
      // Mock FTS5查询返回候选ID
  ...
  describe('getRecentMemoriesFromDB() - 最近记忆检�?, () => {
    it('应该成功获取最近记�?, async () => {
      const mockRows = [
        {
          id: 'mem-recent-1',
